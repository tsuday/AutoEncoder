...
with nn.sess as sess:
    ...
    try:
        while not coord.should_stop():
            ...
            image_data = image_data.reshape((AutoEncoder.batch_size, AutoEncoder.nPixels))
            ...
            if i==start+1 or i % 200 == 0: #200
                ...
                x_input = tf.reshape(x_input, [AutoEncoder.batch_size, nn.outputWidth, nn.outputHeight])
                t_cmp = tf.reshape(t_cmp, [AutoEncoder.batch_size, nn.outputWidth, nn.outputHeight])
                out = tf.reshape(out, [AutoEncoder.batch_size, nn.outputWidth, nn.outputHeight])
                if i==start+1 or i % 1000 == 0: #1000
                    draw_image([x_input.eval(session=nn.sess), out.eval(session=nn.sess), t_cmp.eval(session=nn.sess)],
                               ...
                               AutoEncoder.batch_size, nn.outputWidth, nn.outputHeight)
